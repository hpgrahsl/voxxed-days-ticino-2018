...
public class StateStoreService {
    ...
    public StateStoreService(KafkaStreams kafkaStreams, KafkaProperties kafkaProperties) {
        this.kafkaProperties = kafkaProperties;
        this.kafkaStreams = kafkaStreams;
        String[] parts =  kafkaProperties.getStreams().getProperties()
                                .get(StreamsConfig.APPLICATION_SERVER_CONFIG).split(":");
        ...
        ReceiverOptions<String,Long> rOptsCounts = ReceiverOptions.<String,Long>create(props)
                ...
                .subscription(Collections.singleton(kafkaProperties.getStreams().getProperties().get("changelog-emoji-counts")));
        ...
        hotSourceEmojiCounts = KafkaReceiver.create(rOptsCounts).receive()
                .doOnSubscribe(subscription -> LOGGER.info("reactively subscribed to {} topic",
                        kafkaProperties.getStreams().getProperties().get("changelog-emoji-counts")))
                .publish();
        ...
        ReceiverOptions<String,String> rOptsTweets = ReceiverOptions.<String,String>create(props)
                ...
                .subscription(Collections.singleton(kafkaProperties.getStreams().getProperties().get("emoji-tweets-topic")));

        hotSourceEmojiTweets = KafkaReceiver.create(rOptsTweets).receive()
                .doOnSubscribe(subscription -> LOGGER.info("reactively subscribed to {} topic",
                        kafkaProperties.getStreams().getProperties().get("emoji-tweets-topic")))
                .publish();
    ...
    public Mono<ResponseEntity<EmojiCount>> querySingleEmojiCount(String code) {
        try {
            StreamsMetadata metadata = kafkaStreams.metadataForKey(
                    kafkaProperties.getStreams().getProperties().get("state-store-emoji-counts"),
                code, Serdes.String().serializer());

            if(itsMe.equals(metadata.hostInfo())) {
                ReadOnlyKeyValueStore<String,Long> kvStoreEmojiCounts =
                    kafkaStreams.store(kafkaProperties.getStreams().getProperties().get("state-store-emoji-counts"),
    ...
    public Flux<EmojiCount> queryAllEmojiCounts() {
        Stream<Flux<EmojiCount>> queries =
                        Stream.concat(Stream.of(queryLocalEmojiCounts()),
                            kafkaStreams.allMetadataForStore(kafkaProperties.getStreams().getProperties().get("state-store-emoji-counts")).stream()
    ...
    public Mono<Set<EmojiCount>> queryEmojiCountsTopN() {
        try {
            StreamsMetadata metadata = kafkaStreams.metadataForKey(
                    kafkaProperties.getStreams().getProperties().get("state-store-emojis-top-n"),
            ...
            if(itsMe.equals(metadata.hostInfo())) {
                ...
                ReadOnlyKeyValueStore<String,TopEmojis> kvStoreEmojisTopN =
                    kafkaStreams.store(kafkaProperties.getStreams().getProperties().get("state-store-emojis-top-n"),
                        QueryableStoreTypes.keyValueStore());
